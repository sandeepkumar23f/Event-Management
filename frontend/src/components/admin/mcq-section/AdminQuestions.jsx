...
export default function AdminQuestions() {
  ...
  const deleteQuestion = async (id) => {
    try {
      const res = await fetch(`http://localhost:5000/delete/${id}`, { method: "DELETE", credentials: "include" });
      const data = await res.json();
              ...
              <div>
                ...
                <ul className="text-gray-600 text-sm space-y-1">
                  {qn.options.map((opt, i) => {
                    ...
                    const isCorrect = optionLabel === qn.correctOption || opt === qn.correctOption;
                    return (
                      <li
                        key={i}
                      ...
                      >
                        ...
                        <span>{opt}</span>
                        {isCorrect && <span className="ml-2 bg-green-100 text-green-800 px-2 py-0.5 rounded-full text-xs">Correct</span>}
                      </li>
                    );
                  })}
                </ul>
